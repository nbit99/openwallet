...
`,
	...
	FullnodeContainerConfigs = map[string]*FullnodeContainerConfig{
		...
		"ltc": &FullnodeContainerConfig{ // Release0929
			...
			APIPORT:  []string{"9360/tcp"},
			IMAGE:    string("openw/zcash:v0.16.0"),
			ENCRYPT:  []string{"zcash-cli", "-datadir=/data", "-conf=/etc/zcash.conf", "encryptwallet 1234qwer"},
			STOPCMD:  []string{"zcash-cli", "-datadir=/data", "-conf=/etc/zcash.conf", "stop"},
